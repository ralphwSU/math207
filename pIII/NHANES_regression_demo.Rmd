---
title: "NHANES Regression Examples"
output: pdf_document
---
...
From the output we determine that the regression line is
$$\mbox{recumbent length} = 0.973\times \mbox{measured height} + 3.601$$
...
From the output we determine that the regression line is
$$\mbox{weight} = 0.8928\times \mbox{measured height} -76.28$$
This equation suggests that taller people in the study tended to have longer recumbent length.
...
x2 <- na.omit(x1)
x3 <- x2[(x2$BMPHT != 88888) & (x2$BMPHEAD != 8888), ]
```
...
From the output we determine that the regression line is
$$\mbox{head circumference} = 0.0953\times \mbox{measured height} + 40.15$$
...
x2 <- na.omit(x1)
x3 <- x2[(x2$HSAGEIR != 88888) & (x2$BMPBMI != 8888), ]
```
...
From the output we determine that the regression line is
$$\mbox{bmi} = 0.139\times \mbox{age} + 19.22$$
...
From the output we determine that the regression line is
$$\mbox{red cell count} = 0.0113\times \mbox{white cell count} + 4.542$$
